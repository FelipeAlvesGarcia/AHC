  ...
  .then(dados=>{
    ...
    for (i = 0; i <= 249; i++) 
    {
      if (i != 129 && i!= 83)
      {
        ...
        oficial.textContent = "Official name: " + dados[i].name.official
        ...
        area.textContent = "AREA: " + dados[i].area + "km²"
        ...
        {
          var capital = document.createElement("p")
          capital.textContent = "CAPITAL: does not have"
          div11.appendChild(capital)
        }

        var carros = document.createElement("p")
        carros.textContent = "CARS: " + dados[i].car.side
        ...
        continente.textContent = "CONTINENT: " + dados[i].continents[0]
        ...
        if (i != 44)
        {
          ...
          lingua.textContent = "LANGUAGES: " + linguas
          div12.appendChild(lingua)
        }
        else
        {
          var lingua = document.createElement("p")
          lingua.textContent = "LANGUAGES: does not have"
          div12.appendChild(lingua)
        }

        var popu = document.createElement("p")
        popu.textContent = "POPULATION: " + dados[i].population
        div12.appendChild(popu)

        var sigla = document.createElement("p")
        sigla.textContent = "ACRONYM: " + dados[i].cca2
        ...
        sema.textContent = "START OF THE WEEK: " + dados[i].startOfWeek
        div13.appendChild(sema)

        var regi = document.createElement("p")
        regi.textContent = "REGION: " + dados[i].region
        div13.appendChild(regi)

        var sregi = document.createElement("p")
        sregi.textContent = "SUB REGION: " + dados[i].subregion
        div13.appendChild(sregi)

        var inde = document.createElement("p")
        inde.textContent = "INDEPENDENCE: " + dados[i].independent
      ...
      .then((dados) => {
          ...
          for (let i = 0; i < dados.length; i++) {
              ...
              if (nomePais.includes(termoPesquisa) && i != 129 && i != 83) 
              {
                ...
                oficialP.textContent = "Official name: " + dados[i].name.official
                ...
                areaP.textContent = "AREA: " + dados[i].area + "km²"
                ...
                {
                  var capitalP = document.createElement("p")
                  capitalP.textContent = "CAPITAL: does not have"
                  div11P.appendChild(capitalP)
                }

                var carrosP = document.createElement("p")
                carrosP.textContent = "CARS: " + dados[i].car.side
                div11P.appendChild(carrosP)

                var mapaP = document.createElement("a")
                mapaP.textContent = "MAP"
                ...
                continenteP.textContent = "CONTINENT: " + dados[i].continents[0]
                ...
                if (i != 44)
                {
                  ...
                  linguaP.textContent = "LANGUAGES: " + linguasP
                  div12P.appendChild(linguaP)
                }
                else
                {
                  var linguaP = document.createElement("p")
                  linguaP.textContent = "LANGUAGES: does not have"
                  div12P.appendChild(linguaP)
                }

                var popuP = document.createElement("p")
                popuP.textContent = "POPULATION: " + dados[i].population
                div12P.appendChild(popuP)

                var siglaP = document.createElement("p")
                siglaP.textContent = "ACRONYM: " + dados[i].cca2
                ...
                semaP.textContent = "START OF THE WEEK: " + dados[i].startOfWeek
                div13P.appendChild(semaP)

                var regiP = document.createElement("p")
                regiP.textContent = "REGION: " + dados[i].region
                div13P.appendChild(regiP)

                var sregiP = document.createElement("p")
                sregiP.textContent = "SUB REGION: " + dados[i].subregion
                div13P.appendChild(sregiP)

                var indeP = document.createElement("p")
                indeP.textContent = "INDEPENDENCE: " + dados[i].independent
